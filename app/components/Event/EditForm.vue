    ...
    <UCard variant="soft" class="bg-white rounded-lg px-6 pb-6 shadow-sm">
        ...
        <UForm class="flex flex-col gap-5" @submit.prevent="handleSubmit">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <UFormField>
                    <UInput v-model="dateModel" type="date" placeholder="Fecha" class="w-full" variant="none"
                        :ui="{ base: baseUIClasses }" />
                </UFormField>
                <UFormField>
                    <UInput v-model="form.location" placeholder="Ubicación" class="w-full" variant="none"
                        :ui="{ base: baseUIClasses }" />
                </UFormField>
            </div>
